...
export default {
  ...
  methods: {
    ...
    getCaptcha () {
      ...
      this.$http({
        url: this.$http.adornUrl('/auth/captcha'),
        method: 'get',
        ...
        if (data && data.code === 200) {
          this.captchaPath = 'data:image/png;base64,' + data.data.img
          this.dataForm.realKey = data.data.realKey
        } else {
          this.$message.error(data.msg)
        }
      })
