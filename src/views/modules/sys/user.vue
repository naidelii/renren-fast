...
export default {
  ...
  methods: {
    ...
    getDataList() {
      ...
      this.$http({
        url: this.$http.adornUrl('/sys/user/listPage'),
        method: 'get',
    ...
    // 删除
    deleteHandle(id) {
      const userIds = id ? [id] : this.dataListSelections.map(item => item.id);
      this.$confirm(
        ...
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => this.deleteUsers(userIds)
      ).catch(error => console.error('Delete cancelled', error));
    },
    deleteUsers(userIds) {
      this.$http({
